...
from paper_radar.sources import enrich_with_semantic_scholar
...
def main() -> None:
    ...
    papers, logs = enrich_with_semantic_scholar(papers, configs)
    del logs
    write_jsonl(DATA_DIR / "daily_candidates.jsonl", papers)
    print(f"Enriched {len(papers)} candidates with Semantic Scholar")
